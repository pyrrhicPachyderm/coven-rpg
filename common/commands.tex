...
\makeatletter
\newcommand\herbref[2][\@nil]{%Takes two arguments, the first of which is optional and defaults to nothing.
	\def\herbname{#1}
	\nameref{herb:#2}%
	\ifx\herbname\@nnil%
		%
...
\newcommand\herbrefplural[2][\@nil]{%Takes two arguments, the first of which is optional and defaults to nothing.
	\def\herbname{#1}
	\nameref{herbs:#2}%
	\ifx\herbname\@nnil%
		%
...
\newcommand\mix[3]{%Arguments: creation, delivery, ingedients
	\textbf{\nameref{mixcreation:#1}}, \textbf{\nameref{mixdelivery:#2}}, \materials{#3}
}
