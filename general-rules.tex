...
When the GM determines that the game should move into Structured Time, Initiative Tests are used to determine the order in which participants take their Turns.
...
If she takes the \actionref{dash} Action, she may move a total number equal to twice her Speed.
...
\secrefraw{difficult-terrain}, such as dense forest or a bog, slows characters trying to move through it.
As a simple default, movement through it is halved; it costs 2 metres from a character's Speed to move through 1 metre of \secrefraw{difficult-terrain}.
The GM is free to impose a lesser or greater penalty for more or less severe terrain.

For some kinds of \secrefraw{difficult-terrain}, the GM may offer players the option to ignore the movement penalty at an alternative cost.
...
A character's Shock Threshold is equal to 12, plus their \attref{might}, plus their \attref{will}.
...
Instead, it applies two effects to the target, \secrefraw{shock} and \secrefraw{damage}.
\secrefraw{shock} is always tested for before \secrefraw{damage} is applied.
...
Critical success on a \seclink{Damage Test}{damage-tests} may immediately kill the target or leave them with a lasting injury, at the GM's option, and always applies \secrefraw{shock}.

\subsection{Shock}
\seclabel{shock}

If a \seclink{Damage Test}{damage-tests} meets or exceeds the target's Shock Threshold, or critically succeeds, the target goes into \secrefraw{shock}.
A character in \secrefraw{shock} falls unconscious and cannot be roused while they remain in \secrefraw{shock}.
If a character in \secrefraw{shock} would go into \secrefraw{shock} again due to another \seclink{Damage Test}{damage-tests}, they die.

Additionally, at the start of each of the \seclink{Shocked}{shock} character's Turns, roll a special Test against them.
This Test applies no flat bonus, and uses the same number of dice as the \seclink{Damage Test}{damage-tests} that sent the character into \secrefraw{shock}: a character is more likely to bleed out from a sword wound than a punch.
If it meets or exceeds the \seclink{Shocked}{shock} character's Shock Threshold, they die.
This Test is not considered to be a Test made by any character.

If the Test made every Turn ever totals 9 or less, unless it also meets or exceed their Shock Threshold, the character is no longer in \secrefraw{shock}.
However, the character remains unconscious and cannot be naturally roused before the end of the Scene.
A character can also be brought out of \secrefraw{shock} by another character tending to them.
This requires an Action and a successful \testtype{ken}{healing} Test.
The TN for this Test is 3 times the number of dice that would be rolled against the \seclink{Shocked}{shock} character each round.

\subsection{Damage}
\seclabel{damage}

After \secrefraw{shock} has been tested for, whether or not it occurs, the \seclink{Damage Test}{damage-tests} causes \secrefraw{damage}.
To calculate \secrefraw{damage}, divide the result of the \seclink{Damage Test}{damage-tests} by the target's Resilience.
For example, if the result of the \seclink{Damage Test}{damage-tests} is 13 and the damaged creature has 3 Resilience, they suffer 4 \secrefraw{damage}.
\secrefraw{damage} accumulates: a character who has previously suffered 3 \secrefraw{damage} and suffers an additional 2 is now suffering from 5 \secrefraw{damage}.

\secrefraw{damage} has two effects.
Firstly, a character subtracts their current \secrefraw{damage} from their Shock Threshold.

Secondly, if a character's Shock Threshold ever reaches zero, they die immediately.
This is very unlikely to happen through repeated \secrefraw{damage}, as an earlier blow would send them into \secrefraw{shock}, but can occur if a lot of painkillers wear off all at once.
%Secondly, if a character's current \secrefraw{damage} ever equals or exceeds their original Shock Threshold (unmodified by \secrefraw{damage}), they die immediately.
%This applies even if they are ignoring the effect of some of their \secrefraw{damage}, such as through painkillers.
%As effects that allow a character to ignore \secrefraw{damage} are so common, it can be helpful to track a character's actual \secrefraw{damage} and the \secrefraw{damage} they are considered to be suffering from separately.
...
\secrefraw{damage} heals naturally over time, but it's a slow process.
Once per day, with a decent meal and at least about six hours of sleep, a character may recover from 1 point of \secrefraw{damage}.
If the character spends the entire day resting, they may heal 1 additional point of \secrefraw{damage}.
...
The GM should adjudicate this, taking into account how the \secrefraw{damage} was sustained, but in general a character who is not suffering any penalty to rolls due to \secrefraw{damage} doesn't need to be too careful.
...
For each rank their physician has in the \skillref{healing} skill, a character taking a day of rest may heal 1 additional point of \secrefraw{damage}.
...
Besides injury, an active witch runs the risk of \secrefraw{exhaustion}.
From late night vigils to running after tricksy spirits, many things can leave a witch tired and longing for her bed.

When a character performs something exhausting, or goes a day without at least 6 hours of sleep, the GM may apply a level of \secrefraw{exhaustion}, or call for a Test (typically \attref{might} or \attref{will}) to avoid one.
Each level of \secrefraw{exhaustion} reduces two of a character's attributes by 1.
The GM selects appropriate attributes depending on the type of \secrefraw{exhaustion}.
For example, \secrefraw{exhaustion} as a result of a long foot chase might decrease \attref{might} and \attref{grace}.
Sleep deprivation might decrease \attref{wit} and \attref{heed}.
A long day of socialising, rushing from meeting to meeting, mmight even reduce \attref{charm} and \attref{presence}.

Multiple levels of \secrefraw{exhaustion} may reduce the same attribute, leading to a total reduction of 2 or more.
Whenever a character suffers a second level of \secrefraw{exhaustion} affecting the same attribute, the GM may call for a Test to avoid passing out until they can sleep it off.

A character may reduce their \secrefraw{exhaustion} by 1 level when they get a good night's sleep: about eight hours.
An entire day of rest reduces \secrefraw{exhaustion} by another level.
The player my choose which to attributes to recover when they reduce their \secrefraw{exhaustion}.
...
The Test is made against a TN equal to the target's Dodge Rating: 8, plus their \attref{grace}, plus their \attref{heed}.
