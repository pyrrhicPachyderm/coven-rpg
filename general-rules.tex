...
For example, an argument that boils over into a brawl might prompt Initiative Tests using \testtype{charm}{insight}, favouring characters who noticed tensions rising and fists clenching.
...
If nothing in particular seems appropriate, default to a \attref{grace} Test with no applicable skill.
...
The travellers roll \testtype{wit}{perception} to notice the bandits attacking.
The GM may assign a greater or lesser bonus to a better-laid ambush, or one staged in a suboptimal location.

To save time, the GM may make a single Test for a group of similar NPCs, such that they all get the same result.
...
For example, if the result of the \seclink{Damage Test}{damage-tests} is 13 and the damaged creature has 4 Resilience, they suffer 3 \secrefraw{damage}.
...
\secrefraw{damage} has several effects.
Firstly, a character subtracts their current \secrefraw{damage} from their Shock Threshold.

Secondly, increasing \secrefraw{damage} can cause pain, having an adverse effect upon a character's actions.
For every 3 points by which a character's \secrefraw{damage} exceeds their \attref{will}, they suffer a \negative{1} penalty to all Tests except \seclink{Damage Tests}{damage-tests}.
For example, if a character with 1 \attref{will} is suffering 7 \secrefraw{damage}, their \secrefraw{damage} exceeds their \attref{will} by 6 and they suffer a \negative{2} penalty.
However, if the character had 5 \attref{will}, their \secrefraw{damage} would exceed their \attref{will} by only 2 and they would not suffer any penalty.
...
%Lastly, if a character's current \secrefraw{damage} ever equals or exceeds their original Shock Threshold (unmodified by \secrefraw{damage}), they die immediately.
...
If a creature somehow has a Resilience of 0 or less, any \seclink{Damage Test}{damage-tests} deals \secrefraw{damage} sufficient to drop its Shock Threshold to 0 and kills it.
