...
Some {\portents} use a target: a creature or person about whom information is sought using the {\portent}.
If a {\portent} requires a target, you must watch the target's behaviour while you study the {\portent}.
...
Some {\portents} allow you to use yourself as a target, in which case no observation is necessary.

How the target of a {\portent} is used will depend upon the {\portent}, and will be specified in the relevant feat.
...
For some types of {\portent}, the target may not even appear in the {\vision}.
...
\feat{Seven for a Secret}{portent-birds-2}{15}{
	...
	This works just like \featref{portent-birds}, except that instead of seeing an event that would upset you, you see the event that the target would least like you to see---an event they would want to hide from you.
...
\feat{Portents of Intersection}{portent-visit}{15}{
	...
	Additionally, you must watch the behaviour of a person or other creature: the target.
	You may use yourself as the target.
	
	If you use this {\portent}, you see a {\vision} of the next time the target will visit the location of your {\vision}.
	...
	In most cases---unless you're using \featref{scrying} to watch someone in a different location or \featref{vision-scrying} to see a {\vision} of a different location---the target will already be in the location of the {\vision}.
	In this case, the {\vision} will not just be of the present time: it will wait until the target has left and returned.
}
