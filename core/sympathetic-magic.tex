...
As such, {\tests} to use \discref{sympathetic-magic} tend to use \attref{wit} as their attribute.
...
Make a \testtype{wit}{sympathetic-magic} {\test} {\opposed} by the target's \testtype{will}{sympathetic-magic}.
If the target wins the {\test}, the {\symlink} is broken.
...
Furthermore, note that an object cannot make {\tests}, so a {\standardsymlink} to an object will never be broken by {\stress}.

\section{Feats}
\setfeatdefault{discipline}{sympathetic-magic}{false}
\unsetfeatdefault{tags}

\feat{Object Linking}{symlink-object}{10}{
	\Anyfeatdiscipline{sympathetic-magic}
}{
	...
	If it not specified, whether an effect works against an object is left up the GM, who may call for a {\test} to make it work.
}
...
\feat{Strength in Size}{symlink-strong-size}{20}{
	\skillref[1]{sympathetic-magic},
	\anyfeatdiscipline{sympathetic-magic}
}{
	\capital{\symlinks} are powered by the similarity between the {\symbol} and target.
	The more similar they are, the stronger the {\symlink} can be made.
	By making the {\symbol} similar in \emph{size} to the target, the {\symlink} can be made nigh-unbreakable.
	
	If the {\symbol} is the same size as the target when you establish a {\symlink} with the {\methodofcontagion}, you can establish a {\strongsymlink}.
}
...
\feat{I Bind You}{symlink-declare}{20}{
	...
	Against people that can understand your speech, establishing a {\symlink} this way is easy, and does not normally require a {\test}.
...
\feat{Thrice Bound}{symlink-declare-stable}{15}{
	\skillref[1]{sympathetic-magic},
	\featref{symlink-declare},
	\featref{symlink-stable-duration}
}{
	\featref{symlink-stable-duration} is unusable when establishing a {\symlink} through declaration (\featref{symlink-declare}).
	But, by tripling your declaration, you can achieve the same effect.
	
	After you establish a {\symlink} using \featref{symlink-declare}, you may stabilise it so that it lasts indefinitely, rather than expiring over time.
	This requires you to repeat the {\action} that established the link twice more, showing the {\symbol} to the target and making them expect the link.
	The three declarations do not need to happen on consecutive {\rounds}, but must occur before the {\symlink} expires---within 10 minutes.
}

\feat{Declaration of Strength}{symlink-declare-strong-size}{15}{
	\skillref[2]{sympathetic-magic},
	\featref{symlink-declare},
	\featref{symlink-strong-size}
}{
	You may use \featref{symlink-strong-size} to make a {\strongsymlink} even when using \featref{symlink-declare}, instead of the {\methodofcontagion}.
}
...
\feat{Sympathetic Jerk}{sympathetic-puppet}{15}{
	...
	Disruptions like those listed above do not require a {\test}, but if the outcome is in doubt, the GM may call for an {\opposedtest}.
...
\feat{Sympathetic Puppet}{sympathetic-puppet-2}{25}{
	...
	The manipulator suffers a \negative{6} penalty to any \emph{physical} {\tests} they must make on the target's behalf.
	These {\tests} typically use \attref{grace}, to finely manipulate the {\symbol}, and whichever skill would be used for performing the {\action} normally.
	However, \skillrefspeciality{performance}{Puppetry} can be used in place of the normal skill.
}

\feat{Sympathetic Vomiting}{sympathetic-vomit}{15}{
	\skillref[1]{sympathetic-magic},
	\featref{sympathetic-puppet}
}{
	When a {\symbolpossessive} gag reflex is triggered, you can cause the target to vomit.
	This typically means sticking a finger down the {\symbolpossessive} throat---which requires that the {\symbol} has a throat---and requires an {\action}.
	The target loses the contents of their stomach, and their next {\turn}.
	This acts quickly enough to be effective against ingested mixtures that take effect in just 1 {\round}.
	
	Using this {\stresses} the {\symlink}.
	
	If the target doesn't wish to throw up, make a \testtype{wit}{sympathetic-magic} {\test}, opposed by a \attref{might} or \attref{will} {\test} (with no applicable skill) from the target.
	A target with an empty stomach---perhaps because they just threw up---gets a large bonus on this {\test}.
	If the target succeeds, they won't throw up or lose their next {\turn}, but it's still intensely unpleasant for them.
}
...
\feat{Sympathetic Insomnia}{sympathetic-sleep-deprive}{15}{
	...
	If the {\symbol} is subjected to loud noises, bright lights, stony bedding, or other significant discomforts while the target sleeps, their sleep will be fitful and restless.
	Such sleep does not help them recover from {\damage} or {\exhaustion}, although they may still recover 1 point of {\damage} and 1 level of {\exhaustion} from a {\dayofrest}.
...
\feat{Sympathetic Narcolepsy}{sympathetic-sleep-cause}{15}{
	...
	After this minute, make a \testtype{wit}{sympathetic-magic} {\opposed} by the target's \testtype{will}{sympathetic-magic} {\test}.
...
\feat{Sympathetic Malady}{sympathetic-attribute-reduce}{10}{
	...
	You may afflict a target with various maladies through a {\symlink}.
...
\feat{Sympathetic Knot}{symlink-knot}{15}{
	...
	This means that every {\symlink} in the knot responds to {\stress} as the weakest link in the knot---this may require multiple {\tests} if there are multiple {\standardsymlinks} in the knot.
}
...
\feat{Sympathetic Unknot}{symlink-knot-undo}{10}{
	...
	Untying a knot is harder than tying it in the first place, but you can finally manage it.
