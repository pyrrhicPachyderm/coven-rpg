...
\feat{Sympathetic Puppet}{sympathetic-puppet-2}{25}{
	...
	As an {\action}, you can puppet a target by manipulating its linked {\symbolpossessive} limbs.
	You take a physical {\action} on behalf of the target, which may be moving up to its \statref{speed} using the \actionref{dash} {\action}.
	...
	You suffer a \negative{6} penalty to any \emph{physical} Tests you must make on the target's behalf.
	These Tests typically use \attref{grace}, to finely manipulate the {\symbol}, and whichever skill you would use if you were performing the {\action} normally.
	However, you may use \skillrefspeciality{performance}{Puppeteer} in place of the normal skill.
}
