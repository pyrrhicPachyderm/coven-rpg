...
\feat{Brew of Claws}{weapon-potion}{15}{
	...
	Until then the creature rolls additional dice for \weaponref{unarmed} {\damagetests} using the clawed appendages.
	
	A creature without any effective attacks rolls 2 dice, while a creature that with an existing attack rolls at least 3 dice.
...
\feat{Hedgehog Hide}{resilience-potion-spines}{10}{
	...
	The spines grant the affected creature the same bonuses to \weaponref{unarmed} {\damagetests} as the \featref{weapon-potion}.
	...
	Furthermore, when a creature with these spines is hit by an unarmed \actionref{attack}, the attacker suffers a \dice{2} {\damagetest}.
}
...
\feat{Berserker Broth}{rage-potion}{15}{
	...
	They retain their full knowledge and capabilities, but turn it all to killing.
	They still have a sense of self-preservation, but it is a second priority, behind murder.
	
	There is a sense of urgency to the need to murder, so the drinker will always make the attempt immediately, rather than laying plans that wouldn't take effect until after the potion has worn off.
...
\feat{Projection Poison}{projection-poison}{20}{
	...
	You can brew a variant of \featref{projection-potion} that is delivered through the bloodstream, rather than the nostrils.
...
\feat{Nettle's Bite}{poison-pain}{15}{
	...
	If this poison is delivered by an \actionref{attack}, the target's \statref{shock-threshold} is treated as being 1 lower against the {\damagetest}.
